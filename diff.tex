...
\noindent\textbf{Completeness.} $\forall x \in L, \forall \omega \in R_L(x)$:
    ...
    \pi) = 1] = 1.$$
\DIFaddbegin \DIFadd{There is an alternate definition of Statistical Correctness, where the probability above is $1 - \mathsf{negl}(n)$ instead of $1$. For this explanation, though, Completeness will be used.
}\DIFaddend 
...
\noindent\textbf{Non-Adaptive Soundness.} $\forall x \not\in L$:
    ...
    = 1] = \mathsf{negl}(k).$$
\DIFaddbegin \DIFadd{If the value of $\sigma$ that is picked is a ``bad" value, then there does not exist a proof $\pi$ for $x$ and $\sigma$. }\DIFaddend The above definition is ``non-adaptive", because it does not allow a cheating
...
\noindent\textbf{(Non-Adaptive) Zero-Knowledge.}
    \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{There }\DIFaddend exists a PPT simulator $S$ such that $\forall x \in L, \omega \in
...
\noindent\textbf{(Adaptive) Zero-Knowledge.}
    \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{There }\DIFaddend exists a PPT simulator split into two stages $S_1, S_2$ such that
...
\begin{theorem}\label{thm:amplify-soundness}
    ...
    construct a NIZK \DIFaddbegin \DIFadd{$(K', P', V')$ }\DIFaddend that is \emph{adaptively sound}.
\end{theorem}
\proof
